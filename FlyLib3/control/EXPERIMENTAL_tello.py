...
class FlyLib3Tello:
    ...
    _control_address: tuple[str, int]
    ...
    def __init__(
        ...
        # Set the address
        self._control_address = (self.control_state_host, self.control_port)
    ...
    def _send_command_without_response(self, command: str):
        ...
        self.control_socket.sendto(command.encode(encoding="utf-8"), self._control_address)
    ...
    def _send_command_with_response(self, command: str, timeout: float = DEFAULT_TIMEOUT, max_retries: int = DEFAULT_MAX_RETRIES):
        ...
        while len(self.responses) < 1:
            ...
            self.logger.info(f"Retry command: {command}")
            self.control_socket.sendto(command.encode(encoding="utf-8"), self._control_address)
    ...
    def _send_control_command_with_response(self, command: str, timeout: float = DEFAULT_TIMEOUT, max_retries: int = DEFAULT_MAX_RETRIES):
        ...
        return "ok" in (self._send_command_with_response(command, timeout, max_retries))
...
if __name__ == "__main__":
    ...
    tello = FlyLib3Tello(log_to_console=True, log_level=logging.DEBUG, log_to_file=True)
    tello.connect()
    tello.takeoff()
